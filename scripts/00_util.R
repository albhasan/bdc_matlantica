...
is_sits_valid <- function(x){
    ...
    res <- x %>%
        dplyr::mutate(n_rows = purrr::map_int(time_series, nrow),
                      n_cols = purrr::map_int(time_series, ncol),
                      n_na   = purrr::map_int(time_series, .count_na)) 
    # %>%
    #     dplyr::select(n_rows, n_cols, n_na) %>%
    #     purrr::map(table)
    ...
    res %>%
        ...
        ensurer::ensure_that(length(unique(.$n_rows)) == 1,
                             err_desc = "Number of time steps don't match!") %>%
...
# Helper function. Plot samples' time series.
f_plot <- function(x){
    x %>%
        ggplot2::ggplot() +
